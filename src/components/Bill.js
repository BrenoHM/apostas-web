...
const Pix = ({amount, clearAmount}) => {
    ...
    const [description, setDescription] = useState('Deposito via boleto.')
    ...
    const handleSubmit = async (e) => {
        ...
        const result = await axios.post('/process_payment_bill', data)
        if( result ) {
            setShowLoading(false);
            let a = document.createElement('a');
            a.target= '_blank';
            a.href= result.data.transaction_details.external_resource_url;
            a.click();
        }
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <div className="mt-10">
                <Button className="!py-4 px-10 bg-[#009EE3]"><BsLock className="mr-2" /> Pagar com Boleto</Button>
            </div>
        </form>
    )
}
